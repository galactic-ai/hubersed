...
``uv run python scripts/build_cont_outlier_sample.py --flow-dir DIR``.
"""
...
from hubersed.detect.cont_flow import TAGS, flow_scores, read_screen, source_class
...
def main(argv=None):
    ...
    p.add_argument(
        "--lines",
        default=str(PATHS["RESULTS"] / "lineEW_flow" / "desi_lines.h5"),
    ...
    p.add_argument(
        "--zmax", type=float, default=0.6, help="drop the high-z end above this redshift"
    )
    p.add_argument(
        "--gaia-list",
    ...
    p.add_argument(
        "--sga-list",
        default=str(PATHS["RESULTS"] / "sga_proximity.csv"),
        help="CSV from contam_screens.py; must exist",
    )
    p.add_argument(
        "--source-class",
    ...
    # external contamination screens (scripts/contam_screens.py)
    ...
    print(
        f"  flagged z<{args.zmin} or z>{args.zmax}={n_z}  gaia={n_gaia}  sga={n_sga}"
    ...
    keep = (
        ...
        if args.keep_flagged
            ...
            and not shred[t]
            and not zcut[t]
            and t not in gaia_flag
            and t not in sga_flag
            and t not in extra_flag
        ]
    )
    ...
    if args.source_class:
        ...
        n_by = {c: sum(v == c for v in cls.values()) for c in ("emission", "weak-em", "continuum")}
    ...
    for t in top:
        i = iv[t]
        print(
            f"{t:>18} {zvac[i]:7.4f} {S['LOGMSTAR'][i]:6.2f} "
