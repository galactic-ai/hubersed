...
The command line entry point is ``scripts/make_alf_input.py``. Line numbers refer to alf
commit 4ef7bb8.
...
# Na D, alf's NaD index band 5876.875-5909.375 A (air) padded about 2 A for vacuum.
DEFAULT_MASK = "5876,5913"


def write_alf_input(tid, out, intervals=DEFAULT_INTERVALS, mask=DEFAULT_MASK):
    """Write the alf input file for one TARGETID and print a summary.

    Parameters
    ----------
    tid : int
        DESI TARGETID of the spectrum.
    out : str or pathlib.Path
        Output file. If its folder is called indata, the sibling results and models
        folders that alf.exe expects are created too.
    intervals : str, optional
        Interval edges in microns, as l1,l2,l1,l2. At most 10 intervals.
    mask : str, optional
        Rest-frame Angstrom edges to set to wgt=0, as l1,l2,l1,l2. An empty string masks
        nothing.
    ...
    ------
    SystemExit
        If no pixel falls in both a requested interval and alf's model range.
    """
    ...
    ed = [float(v) for v in intervals.split(",")]
    ...
    rest = lam[keep]
    med = [float(v) for v in mask.split(",") if v.strip()]
    ...
    print(f"  TARGETID {tid}   z = {z:.7f}   frame: {frame}")
