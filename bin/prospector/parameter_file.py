...
def build_model(redshift: float):
    # build model template
    template = copy.deepcopy(BASE_TEMPLATE)

    # set redshift
    template["zred"]["init"] = redshift
    template["agebins"]["init"] = make_stochastic_agebins(z=redshift)
...
# build noise = None for now

def build_all(spec, unc, mask, redshift):

    obs = build_obs(spec, unc, mask)
    model = build_model(redshift)
    sps = build_sps()
    noise = (None, None)
