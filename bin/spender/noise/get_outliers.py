...
def load_latents(path):
    with h5py.File(path, 'r') as f:
        ...
        if 'target_ids' not in f:
            raise KeyError(f"{path} has no 'target_ids' -- re-encode with the fixed "
...
def main():
    ...
    ap.add_argument('--desi', default='spender_spec_6latent_snr3.h5',
                    help='DESI latent file name (in DATA dir)')
    ap.add_argument('--mock', default='prospector_noise_spec_cue_6latent_snr3.h5',
                    help='mock latent file name (in DATA dir)')
    ap.add_argument('--out', default='desi_outliers_cue_snr3.pt',
                    help='output filename (in RESULTS dir)')
    ap.add_argument('--quantile', type=float, default=0.001,
    ...
    print(f"DESI {tuple(desi_latents.shape)} from {args.desi}  |  mock {tuple(p_l.shape)} from {args.mock}")
    ...
    iso = IsolationForest(n_estimators=300, max_samples=2048,
    ...
    torch.save({"outlier_target_ids": outlier_tid,           
                "scores_desi": torch.tensor(scores_desi),    
                "desi_target_ids": desi_tid,                 
                "threshold": float(threshold),
                "n_latent": int(desi_latents.shape[1]),
                "desi_file": args.desi, "mock_file": args.mock,
                "snr_min": mock_snr_min},
               RESULTS_PATH / args.out)
    print(f"{len(outlier_tid)} outliers -> {args.out}  (TARGETIDs; threshold {float(threshold):.4f}; "
