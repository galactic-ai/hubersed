...
def main():
    ...
    p.add_argument("--split-seed", type=int, default=777,
    ...
    p.add_argument("--dry-run", action="store_true",
                   help="load, split, print shapes, train nothing")
    ...
    src, tgt, names = ((mock, desi, ("mockA", "mockB", "desi")) if a.mode == "selfdist"
    ...
    print(f"mode={a.mode} tag={a.tag} dim={mock.shape[1]} encoder={mock_ckpt}\n"
          f"  train[{names[0]}]={Xtrain.shape[0]}  held[{names[1]}]={Xheld.shape[0]}  "
          f"other[{names[2]}]={Xother.shape[0]}  device={dev}", flush=True)
    if a.dry_run:
        return
    ...
    thr = np.empty(S); c2st = np.empty(S); ks_max = np.empty(S); ks_med = np.empty(S)
    ...
    for i, sd in enumerate(seeds):
        ...
        if not np.isnan(c2st[i]) and c2st[i] > 0.6:
            print(f"    WARNING C-2ST {c2st[i]:.3f} > 0.6: flow does not reproduce its own "
        ...
        np.savez_compressed(
            out_f, mode=a.mode, seeds=seeds[:i + 1],
            lp_train=lp_train[:i + 1], lp_held=lp_held[:i + 1], lp_other=lp_other[:i + 1],
            # split_idx_* index the TRAINING dataset, which is the mocks under
            # selfdist and DESI under reverse -- read them with `mode`.
            names=np.array(names), split_idx_a=ia, split_idx_b=ib,
            desi_tid=desi_tid, thr=thr[:i + 1], c2st=c2st[:i + 1],
            ks_max=ks_max[:i + 1], ks_med=ks_med[:i + 1],
            nll_train=nll_tr[:i + 1], nll_valid=nll_val[:i + 1],
            scaler_mean=scaler.mean_, scaler_scale=scaler.scale_,
            tag=a.tag, method=a.method, encoder=mock_ckpt, split_seed=a.split_seed,
